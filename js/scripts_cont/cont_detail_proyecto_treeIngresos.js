$(function(){
	...
	function getIngresosTree(){

		var pkID_proyecto = $("#id_proyecto").val();

		//-----------------------------------------------------------------------------------------------		

		var consulta_presupuesto = "select gasto_gral.*, actividad.nombre as nom_actividad, actividad.subtotal as subtotal_actividad, actividad.iva as iva_actividad, actividad.total as total_actividad"+ 
		...
		//console.log(consulta_presupuesto);						
	    ...
	    .done(function(data) {	    	
			...
			$('#tree_ingresos').treeview({data: folder_array});
	...
	function getIngresosTreeNo(){

		var pkID_proyecto = $("#id_proyecto").val();

		//-----------------------------------------------------------------------------------------------		

		var consulta_presupuesto = "SELECT * FROM `gasto_gral` WHERE `fkID_proyecto` = "+pkID_proyecto+" AND (fkID_actividad = 0 OR fkID_actividad IS null )";
	    ...
	    .done(function(data) {	    	
	        ...
	        if (data.estado != "Error") {
		        ...
		        $.each(data.mensaje, function(index, val) {
		        	...
		        	//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	        	 		 ...
	        	 		 var file = {
				        	 	text: '<strong>'+val.descripcion+' : </strong> | <strong> Sub-Total: $</strong>'+accounting.formatNumber(val.valor,options_format)+' | <strong> IVA: $</strong>'+accounting.formatNumber(val.iva,options_format)+' | <strong> Total: $</strong>'+accounting.formatNumber(val.total,options_format)+' | ',
				        	 	icon: "glyphicon glyphicon-usd",
				        	 	selectable: false,			        	 		      
				        	 }
			...
			$('#tree_ingresosNo').treeview({data: folder_array});
