$(function(){
	...
	function valida_paso_action(id_proceso){
	    ...
	    .done(function(data) {
	        ...
	        //cons_users(id_proceso);
	        if (paso_reciente == 2) {
	        	alert("Este proceso ya se encuentra en el paso 'aprobado' y no es necesario ejecutarlo de nuevo.");	        	
	        } else{
	        	
	        	var confirma = confirm("Esta segur@ que quiere APROBAR este proceso?");

				if (confirma == true) {
					//define el id del proceso
	...
	$("[name*='aprobar_asignar']").click(function(event) {
		/* Act on the event */
		console.log('aprobar proceso '+$(this).attr('data-id-proceso'));
